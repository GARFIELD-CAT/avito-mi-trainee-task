...
## Усложнения
1. Написаны тесты.  
coverage = 97% http://joxi.ru/DmBaKMkF4eKXlr Проверку покрытия можно сделать ```coverage run --source='api' manage.py test api/tests``` и затем ```coverage report```.  
Если хочется в более красивом виде отчет, то можно использовать ```coverage html```. В папке с проектом появится папка htmlcov в ней нужно запустить index.html
